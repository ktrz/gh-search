...
export class UsersService {
  ...
  private query$ = this.searchParams$.pipe(
    map((variables) =>
      this.apollo.watchQuery<SearchUsersResult, SearchUsersVariables>({
        query: SearchUsers,
        variables,
  ...
  private valueChanges$ = this.query$.pipe(
    switchMap((query) => query.valueChanges)
  );
  private loadedValues$ = this.valueChanges$.pipe(
          ...
          updateQuery: (prev, { fetchMoreResult }) => {
            if (
              ...
              prev.search.pageInfo.endCursor ===
                fetchMoreResult.search.pageInfo.endCursor
            ) {
              return prev;
            }
            return {
  ...
  getUsers(): Observable<User[]> {
    return combineLatest([this.getApiUsers(), this.page$]).pipe(
      map(([users, { pageSize, pageIndex }]) =>
        users.slice(pageIndex * pageSize, (pageIndex + 1) * pageSize)
      )
    );
  }

  getUsersCount(): Observable<number> {
    return this.loadedValues$.pipe(map((data) => data.data.search.userCount));
  }

  isLoading(): Observable<boolean> {
    return this.valueChanges$.pipe(map((data) => data.loading));
  }
  ...
  searchUsers(query: string, after?: string): void {
